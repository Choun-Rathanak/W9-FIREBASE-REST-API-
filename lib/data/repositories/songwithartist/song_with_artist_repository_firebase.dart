...
class SongWithArtistRepositoryFirebase extends SongWithArtistRepository{
  ...
  if(responseSong.statusCode == 200 && responseArtist.statusCode == 200){
     Map<String, dynamic> songJson = json.decode(responseSong.body);  // convert the text string in json to map object
     Map<String, dynamic> artistJson = json.decode(responseArtist.body); // convert the text string in json to map object


    final List<SongArtist> songart = [];  // store the result
    
     songJson.forEach((songId, songValue) {  // loop into songJson to join 
      final artistId = songValue['artistId'];  // get the artistId in song
      final artistValue = artistJson[artistId]; // store the artistId in song that matching in the artistValue to join
      if (artistValue != null) {
       songart.add(
      SongArtist(
      song: Song(
